...
public class AutoPlayGallery extends RelativeLayout {
	...
	private void setupContentView(Context context) {
		...
		int heightPX = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, height,
				getResources().getDisplayMetrics()); // change inch to pix
		indicator.setLayoutParams(new LinearLayout.LayoutParams(
				LinearLayout.LayoutParams.FILL_PARENT, heightPX));
		...
		Gallery.LayoutParams paramGallery = new Gallery.LayoutParams(Gallery.LayoutParams.FILL_PARENT,
	...
	private Animation getTranslateAnimation() {
		...
		TranslateAnimation ta = new TranslateAnimation(Animation.RELATIVE_TO_PARENT, 0, Animation.RELATIVE_TO_PARENT,
				-1.0f, Animation.RELATIVE_TO_PARENT, 0, Animation.RELATIVE_TO_PARENT, 0);
		ta.setDuration(500);
		as.addAnimation(ta);
		return as;
	}
	...
	private void indicatePoint(int position) {
		...
		((ImageView) radioGroup.getChildAt(position)).setImageBitmap(pointPressedBg);
	}
